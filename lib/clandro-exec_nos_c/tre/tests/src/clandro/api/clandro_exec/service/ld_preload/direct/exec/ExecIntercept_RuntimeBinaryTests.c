...
void test__execIntercept__Files(const char* clandroExec_tests_testsPath, const char* currentPath, char* envCurrentPath);
...
void test__execIntercept() {
    ...
    char clandroExec_tests_testsPathBuffer[PATH_MAX];
    const char* clandroExec_tests_testsPath;
    int result = getPathFromEnv(LOG_LEVEL__NORMAL, LOG_TAG,
        "tests_path", ENV__CLANDRO_EXEC__TESTS__TESTS_PATH,
        true, 0, true, true,
        clandroExec_tests_testsPathBuffer, sizeof(clandroExec_tests_testsPathBuffer));
    if (result < 0) {
        exit(1);
    } else if (result == 1 || strlen(clandroExec_tests_testsPathBuffer) < 1) {
        clandroExec_tests_testsPath = CLANDRO_EXEC__TESTS__TESTS_PATH;
    } else {
        clandroExec_tests_testsPath = clandroExec_tests_testsPathBuffer;
    }
    logErrorVVerbose(LOG_TAG, "tests_path: '%s'", clandroExec_tests_testsPath);
    ...
    test__execIntercept__Files(clandroExec_tests_testsPath, currentPath, envCurrentPath);
...
void test__execIntercept__Basic() {
    ...
    runAllExecWrappersTest("clandro-rootfs",
        -1, EACCES, -1, EACCES,
        0, NULL, 0, NULL, 0,
        ".", CLANDRO__ROOTFS, environ,
        NULL);
...
void test__execIntercept__AndroidTools(char* envCurrentPath) {
    ...
    // will be `255`.
    // - https://github.com/cl-andro/ClandroAm/commit/8a08e9bd
...
void test__execIntercept__Files(const char* clandroExec_tests_testsPath, const char* currentPath, char* envCurrentPath) {
    ...
    char* clandroExec__execTestFilesPath = NULL;
    asprintf_wrapper(&clandroExec__execTestFilesPath, "%s/%s",
        clandroExec_tests_testsPath, "lib/clandro-exec_nos_c/tre/scripts/clandro/api/clandro_exec/service/ld_preload/direct/exec/files");
    ...
    if (currentPath == NULL || strlen(currentPath) < 1) {
        if (asprintf(&envNewPath, "%s%s", ENV_PREFIX__PATH, clandroExec__execTestFilesPath) == -1) {
            errno = ENOMEM;
            logStrerrorDebug(LOG_TAG, "asprintf failed for new '%s%s'", ENV_PREFIX__PATH, clandroExec__execTestFilesPath);
            exit(1);
        }
    } else {
        if (asprintf(&envNewPath, "%s%s:%s", ENV_PREFIX__PATH, currentPath, clandroExec__execTestFilesPath) == -1) {
            errno = ENOMEM;
            logStrerrorDebug(LOG_TAG, "asprintf failed for new '%s%s:%s'", ENV_PREFIX__PATH, currentPath, clandroExec__execTestFilesPath);
            exit(1);
        }
    }
    ...
    // but `clandro-exec` will prevent this by returning `-1` from
    ...
    if (android_buildVersionSdk_get() < 23) {
        asprintf_wrapper(&testFilePath, "%s/%s", clandroExec__execTestFilesPath, "print-args-binary");
        ...
        asprintf_wrapper(&testFilePath, "%s/%s", clandroExec__execTestFilesPath, "print-args-binary.sym");
    ...
    asprintf_wrapper(&testFilePath, "%s/%s", clandroExec__execTestFilesPath, "print-args-linux-script.sh");
    ...
    asprintf_wrapper(&testFilePath, "%s/%s", clandroExec__execTestFilesPath, "print-args-linux-script.sh.sym");
    ...
    asprintf_wrapper(&testFilePath, "%s/%s", clandroExec__execTestFilesPath, "print-args-clandro-script.sh");
    runAllExecWrappersTest("print-args-clandro-script.sh",
        expectedReturnValue, expectedErrno, expectedReturnValueP, expectedErrnoP,
        expectedExitCode, expectedOutputRegex, expectedExitCodeP, expectedOutputRegexP, REG_EXTENDED,
        "print-args-clandro-script.sh", testFilePath, environ,
        "goodbye-world", NULL);
    free(testFilePath);

    asprintf_wrapper(&testFilePath, "%s/%s", clandroExec__execTestFilesPath, "print-args-clandro-script.sh.sym");
    runAllExecWrappersTest("print-args-clandro-script.sh.sym",
        expectedReturnValue, expectedErrno, expectedReturnValueP, expectedErrnoP,
        expectedExitCode, expectedOutputRegex, expectedExitCodeP, expectedOutputRegexP, REG_EXTENDED,
        "print-args-clandro-script.sh.sym", testFilePath, environ,
        "goodbye-world", NULL);
    free(testFilePath);
    ...
    free(clandroExec__execTestFilesPath);
    free(envNewPath);
...
void test__execIntercept__PackageManager() {
    ...
    char* clandroPackageManager = getenv(ENV__CLANDRO_ROOTFS__PACKAGE_MANAGER);
    if (clandroPackageManager == NULL || strlen(clandroPackageManager) < 1) {
        logStrerrorVerbose(LOG_TAG, "Not running 'package-manager' 'exec()' wrapper tests since '%s' environment variable not set",
            ENV__CLANDRO_ROOTFS__PACKAGE_MANAGER);
        return;
    }

    char* clandroPackageManagerPath = NULL;
    asprintf_wrapper(&clandroPackageManagerPath, "%s/bin/%s", CLANDRO__PREFIX, clandroPackageManager);

    // In case bootstrap was built without a package manager.
    if (access(clandroPackageManagerPath, X_OK) != 0) {
        logStrerrorVerbose(LOG_TAG, "Not running 'package-manager' 'exec()' wrapper tests since failed to access package manager executable path '%s'",
            clandroPackageManagerPath);
        free(clandroPackageManagerPath);
        errno = 0;
        return;
    }
    ...
    char* clandroPackageManagerVersionRegex = NULL;
    if (asprintf(&clandroPackageManagerVersionRegex, "^.*%s v?[0-9][.][0-9][.][0-9].*$", clandroPackageManager) == -1) {
        errno = ENOMEM;
        logStrerrorDebug(LOG_TAG, "asprintf failed for new '^.*%s v?[0-9][.][0-9][.][0-9].*$'", clandroPackageManager);
        exit(1);
    }

    runAllExecWrappersTest("package-manager-version",
        0, 0, 0, 0,
        0, clandroPackageManagerVersionRegex, 0, clandroPackageManagerVersionRegex, REG_EXTENDED | REG_ICASE,
        clandroPackageManager, clandroPackageManagerPath, environ,
        "--version", NULL);

    free(clandroPackageManagerVersionRegex);
