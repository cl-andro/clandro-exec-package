...
void test__inspectFileHeader__Basic() {
    ...
    struct ClandroFileHeaderInfo info = {.interpreterArg = NULL};

    snprintf(header, hsize, "#!/bin/sh\n") < 0 ? abort() : (void)0;
    inspectFileHeader(CLANDRO__PREFIX, header, sizeof(header) , &info);
    state__ATrue(!info.isElf);
    state__ATrue(!info.isNonNativeElf);
    string__AEqual(CLANDRO__PREFIX__BIN_DIR "/sh", info.interpreterPath);
...
void test__modifyExecEnv__setProcSelfExe() {
    ...
    {
    char *clandroProcSelfExe = NULL;
    state__ATrue(asprintf(&clandroProcSelfExe, "%s%s", ENV_PREFIX__CLANDRO_EXEC__PROC_SELF_EXE, CLANDRO__PREFIX__BIN_DIR "/bash") != -1);

    char *testEnv[] = {"MY_ENV=1", NULL};
    char **allocatedEnvp;
    modifyExecEnv(testEnv, &allocatedEnvp, &clandroProcSelfExe, false, false);
    state__ATrue(allocatedEnvp != NULL);
    string__AEqual(allocatedEnvp[0], "MY_ENV=1");
    string__AEqual(allocatedEnvp[1], ENV__CLANDRO_EXEC__PROC_SELF_EXE "=" CLANDRO__PREFIX__BIN_DIR "/bash");
    free(clandroProcSelfExe);
    free(allocatedEnvp);
    }
    ...
    char *testEnv[] = {"MY_ENV=1", ENV__CLANDRO_EXEC__PROC_SELF_EXE "=" CLANDRO__PREFIX__BIN_DIR "/python", NULL};
    char **allocatedEnvp;
    modifyExecEnv(testEnv, &allocatedEnvp, &clandroProcSelfExe, false, false);
    state__ATrue(allocatedEnvp != NULL);
    string__AEqual(allocatedEnvp[0], "MY_ENV=1");
