#ifndef LIBCLANDRO_EXEC__NOS__C__CLANDRO_EXEC_LD_PRELOAD___H
#define LIBCLANDRO_EXEC__NOS__C__CLANDRO_EXEC_LD_PRELOAD___H
 ...
 *
 * A call is made to `clandroExec_systemLinkerExec_mode_get()` to
 ...
 * - `CLANDRO__SE_PROCESS_CONTEXT` or its fallback `/proc/self/attr/current`
 ...
 * `clandro_exec__system_linker_exec__enabled__run_command()` function
 * in `clandro-exec-system-linker-exec`.
 *
 ...
 * `isPathUnderClandroAppDataDir()` returns `true` for the
 * `executablePath`.
 *
 * The executable or interpreter paths are checked under
 * `CLANDRO_APP__DATA_DIR` or `CLANDRO_APP__LEGACY_DATA_DIR` instead of
 * `CLANDRO__ROOTFS` as files could be executed from `CLANDRO__APPS_DIR`
 * and `CLANDRO__CACHE_DIR`, which are not under the Clandro rootfs.
 * Additionally, Clandro rootfs may not exist under app data directory
 ...
 * package with the Clandro rootfs under `/data/local/tmp` instead of
...
#endif // LIBCLANDRO_EXEC__NOS__C__CLANDRO_EXEC_LD_PRELOAD___H
