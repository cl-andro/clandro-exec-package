#ifndef LIBCLANDRO_EXEC__NOS__C__EXEC_INTERCEPT___H
#define LIBCLANDRO_EXEC__NOS__C__EXEC_INTERCEPT___H
 ...
 * terminator.
 *
 * If `libclandro-exec-ld-preload.so` is set in `LD_PRELOAD` and
 * `CLANDRO_EXEC__EXECVE_CALL__INTERCEPT` is enabled, then shebang limit
 * is increased to `340` characters defined by
 * `CLANDRO__FILE_HEADER__BUFFER_SIZE` as shebang is read and script is
 * passed to interpreter as an argument by `clandro-exec` manually.
 ...
 * rootfs is at `/`, so we add Clandro rootfs directory max length to it.
 *
 ...
 *
 * ```
 * LD_DEBUG=3 /data/data/com.clandro/files/usr/libexec/installed-tests/clandro-exec/lib/clandro-exec_nos_c/tre/scripts/clandro/api/clandro_exec/service/ld_preload/direct/exec/files/print-args-binary arg1; echo $?
 * 1
 *
 * # Logcat
 * linker W  [ android linker & debugger ]
 * linker D  DEBUG: library name "/data/data/com.clandro/files/usr/libexec/installed-tests/clandro-exec/lib/clandro-exec_nos_c/tre/scripts/clandro/api/clandro_exec/service/ld_preload/direct/exec/files/print-args-binary" too long
 * ```
 *
 * ```
 * (exec -a print-args-binary /data/data/com.clandro/files/usr/libexec/installed-tests/clandro-exec/lib/clandro-exec_nos_c/tre/scripts/clandro/api/clandro_exec/service/ld_preload/direct/exec/files/print-args-binary arg1); echo $?
 * arg1
 * 0
 * ```
 *
 * ```
 * (cd /data/data/com.clandro/files/usr/libexec/installed-tests/clandro-exec/lib/clandro-exec_nos_c/tre/scripts/clandro/api/clandro_exec/service/ld_preload/direct/exec/files && ./print-args-binary arg1); echo $?
 * arg1
 * 0
 * ```
 *
 ...
 * - https://github.com/cl-andro/clandro-app/issues/213
 *
 * See also `CLANDRO__PREFIX__BIN_FILE___SAFE_MAX_LEN` in
 * https://github.com/cl-andro/clandro-core-package/blob/master/lib/clandro-core_nos_c/tre/include/clandro/clandro_core__nos__c/v1/clandro/file/ClandroFile.h
 *
 * **See Also:**
 ...
 * Default value: `256`
 */
#define CLANDRO__FILE_HEADER__BINPRM_BUF_SIZE 256

/**
 * The max length for interpreter path in the shebang for `clandro-exec`.
 *
 * Default value: `340`
 */
#define CLANDRO__FILE_HEADER__INTERPRETER_PATH___MAX_LEN (CLANDRO__ROOTFS_DIR___MAX_LEN + CLANDRO__FILE_HEADER__BINPRM_BUF_SIZE - 1) // The `- 1` is to only allow one null `\0` terminator.

/**
 * The max length for interpreter arg in the shebang for `clandro-exec`.
 *
 * This is same as `CLANDRO__FILE_HEADER__BINPRM_BUF_SIZE`. There is
 * no way to divide `BINPRM_BUF_SIZE` between path and arg, so we give
 * it full buffer size in case it needs it.
 *
 * Default value: `256`
 */
#define CLANDRO__FILE_HEADER__INTERPRETER_ARG___MAX_LEN CLANDRO__FILE_HEADER__BINPRM_BUF_SIZE

/**
 * The max length for entire shebang line for `clandro-exec`.
 *
 * This is same as `CLANDRO__FILE_HEADER__INTERPRETER_PATH___MAX_LEN`.
 *
 * Default value: `340`
 */
#define CLANDRO__FILE_HEADER__BUFFER_SIZE CLANDRO__FILE_HEADER__INTERPRETER_PATH___MAX_LEN
...
struct ClandroFileHeaderInfo {
    ...
    char interpreterPathBuffer[CLANDRO__FILE_HEADER__INTERPRETER_PATH___MAX_LEN];
    ...
    char interpreterArgBuffer[CLANDRO__FILE_HEADER__INTERPRETER_ARG___MAX_LEN];
};
 ...
 *
 * If `isClandroExecExecveInterceptEnabled()` returns `1`, then
...
/**
 * Inspect file header and set `ClandroFileHeaderInfo`.
 *
 ...
 *               The `CLANDRO__FILE_HEADER__BUFFER_SIZE` should be used
 *               as buffer size when reading.
 * @param headerLength The actual length of the header that was read.
 * @param info The `ClandroFileHeaderInfo` to set.
 */
int inspectFileHeader(const char *clandroPrefixDir, char *header, size_t headerLength,
    struct ClandroFileHeaderInfo *info);
 ...
 * - `/vendor`
 *
 * - https://github.com/cl-andro/clandro-packages/wiki/Clandro-execution-environment#path-environment-variables-exported-by-android
 ...
 * if they are set.
 *
 * @param isNonNativeElf The value for `ClandroFileHeaderInfo.isNonNativeElf`
 ...
 * @param envClandroProcSelfExe If set, then it will overwrite or
 *                             set the `ENV_PREFIX__CLANDRO_EXEC__PROC_SELF_EXE`
 *                             env variable.
...
int modifyExecEnv(char *const *envp, char ***newEnvpPointer,
    char** envClandroProcSelfExe, bool unsetLdVarsFromEnv,
 ...
 * set to `ClandroFileHeaderInfo.origInterpreterPath`, otherwise the
 ...
 * `ClandroFileHeaderInfo.interpreterArg` will be appended if set,
 ...
 * @param info The `ClandroFileHeaderInfo` for the executable file.
...
#endif // LIBCLANDRO_EXEC__NOS__C__EXEC_INTERCEPT___H
