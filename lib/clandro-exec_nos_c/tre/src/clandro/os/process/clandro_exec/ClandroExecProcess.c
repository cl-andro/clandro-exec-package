...
#include <clandro/clandro_core__nos__c/v1/logger/FileLoggerImpl.h>
...
int clandroExec_process_initProcess(const char *versionToLog, const char *logFilePath) {
    ...
    // Android 14 (x86_64).
    // It is not being caused by `clandro-exec` as it happens even
    ...
    // errno if it was set by something else and `clandro-exec` library
    // also has checks to error out if errno is set in various places,
    // like initially in `stringToInt()` called by `clandroExec_logLevel_get()`.
    ...
    return clandroExec_process_initLogger(versionToLog, logFilePath);
}

int clandroExec_process_initLogger(const char *versionToLog, const char *logFilePath) {
    if (sInitLogger) {
        setDefaultLogTagAndPrefix(CLANDRO__LNAME);
        setCurrentLogLevel(clandroExec_logLevel_get());
        setCacheLogPid(true);
        if (logFilePath != NULL) {
        ...
        if (versionToLog != NULL) {
            logErrorVVerbose("", "CLANDRO_EXEC__VERSION: '%s'", versionToLog);
        }
    }
    return 0;
}
