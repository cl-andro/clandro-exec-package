...
/**
 * Intercept and make changes required for clandro and then call
...
int execveIntercept(bool intercept, const char *executablePath, char *const argv[], char *const envp[]) {
    ...
    int result;
    if (clandroExec_execveCall_intercept_get() == 0) {
...
int execveInterceptInternal(const char *origExecutablePath, char *const argv[], char *const envp[]) {
    ...
    //   For instance, `$CLANDRO__PREFIX/bin/ls` is a symlink to `$CLANDRO__PREFIX/bin/coreutils`,
    //   but we need to execute `$CLANDRO__PREFIX/bin/ls` `/system/bin/linker $CLANDRO__PREFIX/bin/ls`
    ...
    //   so that `clandroPrefixPath()` can successfully match it to
    //   replace prefix with clandro rootfs prefix.
    // - For a relative path, we do not replace prefix with clandro rootfs
    //   prefix, but instead prefix the current working directory (`cwd`).
    //   If `cwd` is set to `/bin` and `./sh` is executed, then
    //   `/bin/sh` should be executed instead of `$CLANDRO__PREFIX/bin/sh`.
    ...
    // - $CLANDRO_EXEC__PROC_SELF_EXE will be later set to the processed path
    ...
    if (isFdPath(executablePath)) {
        ...
        // If path is absolute, then normalize first and then replace clandro prefix.
        ...
        executablePath = clandroPrefixPath(LOG_TAG, NULL, executablePath,
            processedExecutablePathBuffer, sizeof(processedExecutablePathBuffer));
        if (executablePath == NULL) {
    ...
    // NOTE: `ClandroFileHeaderInfo.isElf` state will only be valid if
    ...
    char header[CLANDRO__FILE_HEADER__BUFFER_SIZE];
    ...
    //     - https://github.com/cl-andro/clandro-exec-package/issues/31
    // - `/system/bin/*` utilities provided by `toybox`/`toolbox`,
    //   like `ls` or `getprop` on Samsung devices (S25*).
    //     - https://github.com/cl-andro/clandro-app/issues/4448
    //     - https://github.com/cl-andro/clandro-exec-package/issues/32
    // - `/system/bin/app_process`.
    //     - https://github.com/cl-andro/clandro-app/issues/4440#issuecomment-2746002438
    //
    ...
    // Clandro app data directory, and we skip reading file header only
    ...
    // error: "/data/data/com.clandro/files/usr/libexec/installed-tests/clandro-exec/lib/clandro-exec_nos_c/tre/scripts/clandro/api/clandro_exec/service/ld_preload/direct/exec/files/print-args-linux-script.sh" \
    ...
    // error: "/data/data/com.clandro/files/usr/bin/login" \
    ...
    if (shouldEnableSystemLinkerExecResult < 0) {
        ...
        // `clandroApp_dataDir_isPathUnder()` will return following
    ...
    // If `system_linker_exec` is going to be used, then set `CLANDRO_EXEC__PROC_SELF_EXE`
    // environment variable to `processedExecutablePath`, otherwise
    // unset it if it is already set.
    char *envClandroProcSelfExe = NULL;
    if (shouldEnableSystemLinkerExec) {
        modifyEnv = true;
        logErrorVVerbose(LOG_TAG, "set_proc_self_exe_var_in_env: '%d'", true);

        if (asprintf(&envClandroProcSelfExe, "%s%s", ENV_PREFIX__CLANDRO_EXEC__PROC_SELF_EXE, processedExecutablePath) == -1) {
            errno = ENOMEM;
            logStrerrorDebug(LOG_TAG, "asprintf failed for '%s%s'", ENV_PREFIX__CLANDRO_EXEC__PROC_SELF_EXE, processedExecutablePath);
            return -1;
        }
    } else {
        const char *proc_self_exe_var[] = { ENV_PREFIX__CLANDRO_EXEC__PROC_SELF_EXE };
    ...
    char **newEnvp = NULL;
    if (modifyEnv) {
        if (modifyExecEnv(envp, &newEnvp, &envClandroProcSelfExe, unsetLdVarsFromEnv, unsetLdPreloadFromEnv) != 0 ||
            newEnvp == NULL) {
            logErrorDebug(LOG_TAG, "Failed to create modified exec env");
            free(envClandroProcSelfExe);
            return -1;
        }
    ...
    #if defined LIBCLANDRO_EXEC__NOS__C__EXECVE_CALL__CHECK_ARGV0_BUFFER_OVERFLOW && LIBCLANDRO_EXEC__NOS__C__EXECVE_CALL__CHECK_ARGV0_BUFFER_OVERFLOW == 1
...
int inspectFileHeader(const char *clandroPrefixDir, char *header, size_t headerLength,
    ...
    *newlineIndex = 0;
    ...
    if (interpreterPath[0] == '/') {
        ...
        info->interpreterPath = clandroPrefixPath(LOG_TAG, clandroPrefixDir,
...
int modifyExecEnv(char *const *envp, char ***newEnvpPointer,
    ...
    // we might perhaps append a CLANDRO_EXEC__PROC_SELF_EXE variable and
    ...
    *newEnvpPointer = newEnvp;
    ...
    for (int i = 0; i < envCount; i++) {
        if (stringStartsWith(envp[i], ENV_PREFIX__CLANDRO_EXEC__PROC_SELF_EXE)) {
            if (envClandroProcSelfExe != NULL && *envClandroProcSelfExe != NULL) {
                newEnvp[index++] = *envClandroProcSelfExe;
                alreadyFoundProcSelfExe = true;
                if (!isRunningTests) {
                    logErrorVVerbose(LOG_TAG, "Overwrite '%s'", *envClandroProcSelfExe);
                }
            }
            else {
                if (!isRunningTests) {
    ...
    if (envClandroProcSelfExe != NULL && *envClandroProcSelfExe != NULL && !alreadyFoundProcSelfExe) {
        newEnvp[index++] = *envClandroProcSelfExe;
        if (!isRunningTests) {
            logErrorVVerbose(LOG_TAG, "Set '%s'", *envClandroProcSelfExe);
        }
    }
...
int checkExecArg0BufferOverflow(char *const *argv,
    ...
    if (argv0Length >= 128) {
        ...
        if (androidBuildVersionSdk < 23) {
            ...
            char* label = "";
            if (shouldEnableInterpreterExec) {
                char interpreterHeader[CLANDRO__FILE_HEADER__BUFFER_SIZE];
