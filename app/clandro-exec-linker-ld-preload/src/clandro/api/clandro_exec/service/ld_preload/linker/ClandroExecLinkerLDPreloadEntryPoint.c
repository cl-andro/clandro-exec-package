...
/**
 * This file defines functions intercepted by `libclandro-exec-linker-ld-preload.so` using `$LD_PRELOAD`.
 *
 * All exported functions must explicitly enable `default` visibility
 * with `__attribute__((visibility("default")))` as `libclandro-exec-linker-ld-preload.so`
...
#define LIBCLANDRO_EXEC_LINKER_LD_PRELOAD__VERSION_NAME CLANDRO_EXEC_PKG__VERSION
#define LIBCLANDRO_EXEC_LINKER_LD_PRELOAD__VERSION_STRING "libclandro-exec-linker-ld-preload version=" CLANDRO_EXEC_PKG__VERSION " org=" CLANDRO__REPOS_HOST_ORG_NAME " project=clandro-exec-package"



void clandroExec_linkerLdPreload_initProcess() {
    clandroExec_process_initProcess(LIBCLANDRO_EXEC_LINKER_LD_PRELOAD__VERSION_STRING, NULL);
}
...
int execl(const char *name, const char *arg, ...) {
    clandroExec_linkerLdPreload_initProcess();
